...
type SudokuBoardValues = Vec<Vec<SudokuCell>>;

fn main() {
    let initial_board = create_initial_board("board.txt");
    // initial_board[0][0];
    print_board(&initial_board);
    is_valid(&initial_board);
}
...
fn is_valid(board: &SudokuBoardValues) -> bool {
    ...
    board.iter().for_each(|row| {
        row_index += 1;
        if row.len() != 9 {
            println!("Row {row_index} has invalid number of entries");
